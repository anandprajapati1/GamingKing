...
const patchEsm = () => {
    ...
    if (!(win.CSS && win.CSS.supports && win.CSS.supports('color', 'var(--c)'))) {
        // @ts-ignore
        return __sc_import_vautil('./p-8178315f.js');
    }
...
const patchBrowser = async () => {
    ...
    if (importMeta !== '') {
        ...
        if (!window.customElements) {
            // @ts-ignore
            await __sc_import_vautil('./p-59290340.js');
        }
    ...
    if ( (hostRef.$flags$ & 32 /* hasInitializedComponent */) === 0) {
        ...
        if ( !styles.has(scopeId) && Cstr.style) {
            ...
            if ( cmpMeta.$flags$ & 8 /* needsShadowDomShim */) {
                style = await __sc_import_vautil('./p-c68d0961.js').then(m => m.scopeCss(style, scopeId, false));
            }
